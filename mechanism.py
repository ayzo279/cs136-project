...
if __name__ == "__main__":
    ...
    mat = np.zeros((10,10))
    for i, var1 in tqdm(enumerate(range(1,101, 10))):
        for j, var2 in enumerate(range(1, 101, 10)):
            
            student_pref = []
            match1 = []
            for _ in range(200):
                test = CourseMechanism(100, 15, var1=var1)
                test.generate_preferences()
                test.studentDA(10)
                student_pref.append(test.student_preferences.copy())
                match1.append(test.student_matching.copy())


            total = 0
            for e in range(200):
                test.var2 = var2
                test.student_preferences = student_pref[e].copy()
                test.student_matching = match1[e].copy()
                test.resample_preferences(bump=False)
                test.TTC()
                total += enumerate_reassign(match1[e], test.student_matching)
